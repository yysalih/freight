  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          languages[language]!["offer_details"]!,
          style: const TextStyle(color: kWhite),
        ),
        actions: [
          offerProvider.when(
            data: (offer) {
              return offer.loadOwnerUid ==
                      FirebaseAuth.instance.currentUser!.uid
                  ? IconButton(
                      onPressed: () {
                        showDeleteDialog(
                          context: context,
                          title: languages[language]!["delete_offer_title"]!,
                          content:
                              languages[language]!["delete_offer_content"]!,
                          onPressed: () {
                            offerNotifier.deleteOffer(
                              offerUid: offer.uid!,
                            );
                    ...
                    children: [
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    splashRadius: 30,
                                    onPressed: () {
                                      chatNotifier.createChat(context,
                                          to: offer.loadOwnerUid!,
                                          errorTitle: languages[language]![
                                          "error_creating_chat"]!);
                                    },
                                    icon: const Icon(
                                      Icons.chat_bubble,
                      ...
                      Column(
                        ...
                        children: [
                                ...
                                child: Container(
                                  ...
                                  child: Padding(
                                    padding: EdgeInsets.symmetric(
                                        horizontal: 10.0.h, vertical: 25.h),
                                    child: Row(
                                      children: [
                                        Icon(
                                                        ...
                                                        child: Center(
                                                          child: Text(
                                                            [
                                                              "${truck.length} mt",
                                                              "${truck.weight} kg",
                                                              "${languages[language]![truck.isPartial! ? "partial" : "full"]}"
                                                            ][i],
                                                            style: kCustomTextStyle
                                                                .copyWith(
                                                                fontSize: 11.w,
                          ...
                          loadInfoWidget(width, height,
                              title: languages[language]!["description"]!,
                      ...
                      if (offer.state == "rejected")
                        Row(
                          children: [
                            ...
                            Text(
                              languages[language]!["you_have_rejected_this_offer"]!,
                        ...
                        Row(
                          children: [
                            ...
                            Text(
                              languages[language]!["you_have_accepted_this_offer"]!,
                              style: kCustomTextStyle.copyWith(color: Colors.green),
                            ),
                          ],
                        )
                      else
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: carrierProvider.when(
                                data: (carrier) => customButton(
                                  title: languages[language]!["confirm"]!,
                                  color: kGreen,
                                  onPressed: () async {
                                    await offerNotifier.updateOfferState(context,
                                        offerUid: offerUid,
                                        newState: "accepted",
                                        errorTitle: languages[language]![
                                        "error_accept_offer"]!,
                                        successTitle: languages[language]![
                                        "success_accept_offer"]!);

                                    if (offer.type == "load") {
                                      await loadNotifier.updateLoadState(
                                        context,
                                        loadUid: offer.unitUid!,
                                        newState: "accepted",
                                      );
                                      ...
                                      loadProvider.when(
                                          data: (load) {
                                            NotificationService().sendPushMessage(
                                                title: language == "tr"
                                                    ? "${load.originName} - ${load.destinationName} ${languages[language]!["offer_accepted_load_title"]!}"
                                      ...
                                      await truckNotifier.updateTruckPostState(
                                        context,
                                        truckPostUid: offer.unitUid!,
                                        newState: "accepted",
                                      );
                                      ...
                                      truckPostProvider.when(
                                        data: (truckPost) {
                                          NotificationService().sendPushMessage(
                                              title: language == "tr"
                                                  ? "${truckPost.originName} - ${truckPost.destinationName} ${languages[language]!["offer_accepted_truck_title"]!}"
                                                  : "${languages[language]!["offer_accepted_truck_title"]!} ${truckPost.originName} - ${truckPost.destinationName} ",
                                              body: languages[language]![
                                              "offer_accepted_body"]!,
                                              token: offer.loadOwnerUid!,
                                              type: "offer_truck",
                                              uid: offer.unitUid!);
                                        },
                                        error: (error, stackTrace) {},
                                    ...
                                    await shipmentNotifier.createShipment(context,
                                        type: "load",
                                        unitUid: offer.unitUid!,
                                        toUid: offer.carrierUid!,
                                        price: offer.price!,
                                        truckUid: offer.truckUid!,
                                        errorTitle: languages[language]![
                                        "success_creating_shipment"]!,
                                        successTitle: languages[language]![
                            ...
                            SizedBox(
                              width: width * .42,
                              child: customButton(
                                title: languages[language]!["reject"]!,
                                color: Colors.redAccent,
                                onPressed: () {
                                  offerNotifier.updateOfferState(context,
                                      offerUid: offerUid,
                                      newState: "rejected",
                                      errorTitle: languages[language]![
                                      "error_reject_offer"]!,
                                      successTitle: languages[language]![
                                      "success_reject_offer"]!);
