...
class MainState {
  ...
  final TemporaryPlaceModel selectedPlaceModel;
  ...
  MainState({
    ...
    required this.selectedPlaceModel,
  });
  ...
  MainState copyWith({
    ...
    String? placeType,
    TemporaryPlaceModel? selectedPlaceModel,
    ...
    return MainState(
      ...
      selectedPlaceModel: selectedPlaceModel ?? this.selectedPlaceModel,
    );
  }
}
...
class MainController extends StateNotifier<MainState> {
  ...
  List<Map<String, dynamic>> pageInfo = [
    ...
    }, //TODO needs to be replaced with language provider - 30/10/2024 15:04
    {
      "label": "Yüklerim",
  ...
  changePage(int index) {
    state = state.copyWith(
        bottomIndex: index, isLoadExpanded: false, isTruckPostExpanded: false);
  }

  changeExpansions(
      {required bool isTruckPostExpanded, required bool isLoadExpanded}) {
    state = state.copyWith(
        isTruckPostExpanded: isTruckPostExpanded,
        isLoadExpanded: isLoadExpanded);
  }

  changeTruck({required TruckModel value}) =>
      state = state.copyWith(truck: value);
  ...
  getSelectedPlaces({required String placeType}) async {
    List<TemporaryPlaceModel> places =
        await GooglePlacesService.searchPlaces(placeType);
    state = state.copyWith(places: places, placeType: placeType);
  }

  changeSelectedPlace({required bool isPlaceSelected, required TemporaryPlaceModel selectedPlaceModel}) =>
      state = state.copyWith(isPlaceSelected: isPlaceSelected, selectedPlaceModel: selectedPlaceModel);
...
final mainController = StateNotifierProvider<MainController, MainState>(
  (ref) => MainController(
    MainState(
        bottomIndex: 0,
        selectedTab: "",
        isLoadExpanded: false,
        isTruckPostExpanded: false,
        currentUser: UserModel(),
        truck: TruckModel(name: ""),
        load: LoadModel(originName: ""),
        searchString: "",
        filteredItems: [],
        itemsOpened: false,
        placeType: "",
        ...
        selectedPlaceModel: TemporaryPlaceModel(),
    ),
  ),
);
